...
public class ShellExplosion : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        for(int i=0; i < colliders.Length; i++)
        {
            ...
            castle_code castle = targetRigibody.GetComponent<castle_code>();
            if(castle != null)
            {
                float damage_location = CalculateDamage(targetRigibody.position);
                castle.Healthset(damage_location);         
            }
            else if (!targetRigibody)
            {
                continue;
            }
            else if (castle != null)
            {
                float damage_obj = CalculateDamage(targetRigibody.position);
                castle.Healthset(damage_obj);
